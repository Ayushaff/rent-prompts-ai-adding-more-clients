...
import LivePreview from "../PrivateRapp/LivePreview";
import Step3RappDetails from "../PrivateRapp/Step3RappDetails";
import { Model } from "@/payload-types";
import Link from "next/link";
import Step4RappDetails from "../PrivateRapp/Step4RappDetails";
import { ClipLoader } from "react-spinners";
import { useUser } from "@/providers/User";
import { usePathname, useRouter, useSearchParams } from "next/navigation";

interface CreatePublicRapps {
  imageString?: [];
  type: string;
  model: string;
  modelName: string;
...
const Publicrapp: React.FC = () => {
  ...
  const [formData, setFormData] = useState<CreatePublicRapps>({
    type: "text",
    modelType: "text",
    imageString: [],
    model: "",
  ...
  const maxSteps = user?.role === "enterprise" ? 3: 3;
  const totalSteps = user?.role === "enterprise" ? 3 : 3;

  const nextStep = () => {

    if (
      (user?.role === "enterprise" && step === 4) ||
      (user?.role !== "enterprise" && step === 3)
    ) {
      handleSubmit();
    } else {
      setStep(step + 1);
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="md:p-4 py-4 flex flex-col gap-4 md:bg-indigo-800 md:shadow-2xl relative w-full">
        ...
        <div className="flex  justify-between absolute top-20 md:top-6 md:right-4 gap-4 items-end w-full">
          ...
          {!successMessage && step < maxSteps ? (
            ...
            !successMessage &&
            ((step === 3 && user?.role !== "enterprise") || // Non-enterprise users submit on step 3
              (step === 3 && user?.role === "enterprise")) && // Enterprise users submit on step 4
            (
              <button
                onClick={handleSubmit}
                disabled={isSending}
                className="px-3 md:px-4 py-1 md:py-2 md:mr-3 font-bold bg-green-600 text-white rounded-lg hover:bg-green-700 transition-all"
              >
