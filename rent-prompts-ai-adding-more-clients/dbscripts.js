...
export async function migrateRapps() {
  const uri = 'mongodb://localhost:27017/clone_DB'; // Replace with your MongoDB URI
  const client = new MongoClient(uri);

  try {
    await client.connect();
    const database = client.db('clone_DB'); // Replace with your database name
    ...
    for (const rapp of rapps) {
      ...
      const convertAndExtractVariables = (text, prefix) => {
...
//             {
//                 updateOperations.$set.role='user';
//             }
//             // Update `gender` field to ensure `hasMany: false`
//             if (user.genInfo?.gender && Array.isArray(user.genInfo.gender)) {
//                 updateOperations.$set['genInfo.gender'] = user.genInfo.gender[0] || null; // Take the first value
...
//             //ye niche wala rakhte hain
//             if (!user.userType) {
//                 updateOperations.$set.userType = 'individual'; // Default to "individual" if missing
//             }
